...
3. Each player starts with 100 health and uses **WASD** or the arrow keys. Aim with the mouse and hold the left mouse button to fire. Hits remove 10 health, shown above each player. When one player reaches zero health, the round stops, both players see their win/loss result, and either can select **Start over**. Bullets disappear when they hit a player, wall, or obstacle. The host simulates movement, health, and bullets, then sends their state to all clients.
